...
describe("BottomCta scroll behavior", () => {
  it("is visible by default (data-state=visible)", () => {
    ...
    const region = screen.getByRole("region", { name: "底部操作栏", hidden: true });
  ...
  it("shows again when scrolling UP", () => {
    render(<BottomCta>cta</BottomCta>);
    scrollTo(300);
    expect(screen.getByRole("region", { hidden: true }).getAttribute("data-state")).toBe("hidden");
    scrollTo(200); // up by 100
    expect(screen.getByRole("region", { hidden: true }).getAttribute("data-state")).toBe("visible");
  });
...
describe("BottomCta offset variants", () => {
  it("default offset = above-nav (bottom-16)", () => {
    render(<BottomCta>cta</BottomCta>);
    expect(screen.getByRole("region", { hidden: true }).className).toContain("bottom-16");
  });

  it("offset=bottom uses bottom-0", () => {
    render(<BottomCta offset="bottom">cta</BottomCta>);
    expect(screen.getByRole("region", { hidden: true }).className).toContain("bottom-0");
  });

  it("offset=above-nav-lg uses bottom-20", () => {
    render(<BottomCta offset="above-nav-lg">cta</BottomCta>);
    expect(screen.getByRole("region", { hidden: true }).className).toContain("bottom-20");
  });
});
...
describe("BottomCta animation tokens", () => {
  it("uses spring cubic-bezier transition", () => {
    render(<BottomCta>cta</BottomCta>);
    const cls = screen.getByRole("region", { hidden: true }).className;
