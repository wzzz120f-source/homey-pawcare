...
const SPECS = [
  { label: "规格", options: ["标准装", "大包装", "试用装"] },
  { label: "口味/颜色", options: ["经典款", "升级款", "限量版"] },
];
...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const [merchantRes, reviewRes] = await Promise.all([
  ...
  const handleAddToCart = () => {
    ...
    toast({ title: "已加入购物车", description: `${product.name} x${quantity}` });
  };
      ...
      <main className="max-w-lg mx-auto">
        ...
        <div className="px-4 pt-4 space-y-2">
          <div className="flex items-baseline gap-2">
            <span className="text-primary font-extrabold text-3xl">¥{product.price}</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">已售 {product.sales_count} · 库存 {product.stock}</span>
          </div>
        </div>
        ...
        <div className="px-4 mt-4 space-y-3">
          {SPECS.map((spec) => (
            <div key={spec.label}>
              <p className="text-sm font-semibold text-foreground mb-2">{spec.label}</p>
              <div className="flex flex-wrap gap-2">
                {spec.options.map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setSelectedSpecs((p) => ({ ...p, [spec.label]: opt }))}
                    className={cn(
                      "px-3 py-1.5 rounded-lg text-sm border transition-colors",
                      selectedSpecs[spec.label] === opt
        ...
        <div className="px-4 mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="w-8 h-8 rounded-lg bg-primary text-primary-foreground flex items-center justify-center">
